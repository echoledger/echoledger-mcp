...
> **Superseded at execution (2026-06-13): shipped on Railway, not Smithery.** v0.1 deployed directly to **`https://mcp.defimind.ai`** on Railway (Cloudflare-fronted) — the "Upgrade path" was taken at launch. The branded domain + full control won out, and Railway built the heavy Python container cleanly. `smithery.yaml` + `Dockerfile` are kept valid (future Smithery listing / Cloud Run fallback). The Smithery *listing* becomes a Phase 4 aggregator submission. The decision text below is preserved as the original rationale.
